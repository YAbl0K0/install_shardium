...
while 1 {
    interact {
        -nobuffer -o- "By running this installer, you agree to allow the Shardeum team to collect this data. (Y/n)?:" {send "\r"}
        "What base directory should the node use (default ~/.shardeum):" {send "\r"}
        "Do you want to run the web based Dashboard? (y/n):" {send "\r"}
        "Enter the port (1025-65536) to access the web based Dashboard (default 8080):" {send "8180"}
        ...
        "This allows p2p commnication between nodes. Enter the first port (1025-65536) for p2p comminucation (default 9001):" {send "\r"}
